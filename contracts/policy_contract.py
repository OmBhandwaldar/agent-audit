"""
AgentAudit Phase 2 — Policy Contract.

Stateless enforcer: checks payment policy and mints AACR receipt.
Does NOT store audit records on-chain (records go to batcher → Merkle → AnchorContract).

Policies enforced:
  1. Amount check — payment amount must be below policy limit
  2. Vendor check — vendor must be on the on-chain approved whitelist

On both passing: transfers 1 AACR compliance receipt to caller.
...
class PolicyContract(ARC4Contract):
    """
    AgentAudit Phase 2 policy enforcement contract.
    ...
    Methods:
      ...
      check_and_mint — check both policies, mint AACR if both pass (no record stored)
    """

    def __init__(self) -> None:
        """Declare global state and vendor box map."""
    ...
    def check_and_mint(
        ...
        """
        Check both policies and mint AACR receipt if both pass.

        Policy 1 — Amount check: amount < policy_limit
        Policy 2 — Vendor check: vendor_id is in the approved whitelist

        Does NOT store the audit record on-chain.
        Records are batched off-chain and anchored via AnchorContract.
        ...
        Returns:
            Policy result string: "amount:pass|vendor:pass" (or "fail" per check).
        """
        ...
        # Mint AACR receipt if both policies pass
        if both_pass:
            itxn.AssetTransfer(
                xfer_asset=Asset(self.compliance_asa_id.value),
